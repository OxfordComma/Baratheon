...
public class PhotonPlayer
{
    ...
    public PhotonPlayer GetNextFor(PhotonPlayer localPlayer)
    {
        if (localPlayer == null)
        {
            return null;
        }
        return GetNextFor(localPlayer.ID);
    }

    public PhotonPlayer GetNextFor(int localPlayerId)
    {
        ...
        int lowestId = localPlayerId;     // if we are the player with the highest ID, there is no higher and we return to the lowest player's id
        ...
        foreach (int playerid in players.Keys)
        {
            ...
            else if (playerid > localPlayerId && playerid < nextHigherId)
            {
                nextHigherId = playerid;    // more than our ID and less than those found so far.
            }
        }

        //UnityEngine.Debug.LogWarning("Debug. " + localPlayerId + " lower: " + lowestId + " higher: " + nextHigherId + " ");
        //UnityEngine.Debug.LogWarning(this.RoomReference.GetPlayer(localPlayerId));
